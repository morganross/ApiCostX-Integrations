...
def _system_prompt() -> str:
    return (
        ...
        "model selection, input documents, generation instructions, and run settings. The preset generator named OWL "
        "is an isolated CAMEL-AI Workforce research engine; it is not the separate Allie Owl chatbot API. Its "
        "runtime is configured out of process and currently exposes DuckDuckGo public-web search only, without "
        "browser, shell, or file-writing tools. A registered engine or healthy worker does not prove successful "
        "execution or output quality; use live preset/model/run evidence and do not claim OWL cancellation works "
        "because its adapter cancellation is not implemented. The advanced assistant can "
